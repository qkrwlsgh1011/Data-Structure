...
public:
	...
	void insert_edge(int source, int destination) {
		if (isFindVertex(source) == true && isFindVertex(destination) == true) {
			...
			if (isFindEdge(source, destination) == 1) {
				cout << -1 << endl;
			}
			else {
...
int main() {
	...
	int vertexvalue;
	int A, B;
	cin >> vertexmuch >> edgemuch >> adjacentmuch;
	graph g;
	while (vertexmuch--) {
		cin >> vertexvalue;
		g.insert_vertex(vertexvalue);
	}
	while (edgemuch--) {
		cin >> A >> B;
		g.insert_edge(A, B);
	...
	cout << g.vertexSize << " " << g.TotaledgeList->size << endl;

	while (adjacentmuch--) {
		cin >> A >> B;
		if (g.findVertex(A) == NULL || g.findVertex(B)==NULL) {
			cout << -1 << endl;
		}
		else {
			cout << g.isAdj(A, B) << endl;
		}
	}
