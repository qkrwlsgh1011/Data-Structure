...
void insertHash(int idx) {
	...
	if (HashArr[hashfunc(idx + probing - 1)].flag == NOITEM) {
		HashArr[hashfunc(idx + probing - 1)].key = idx;	//원래자연수 key에 저장
		HashArr[hashfunc(idx + probing - 1)].value = idx + probing - 1;//변환해서 어디에 저장하는지
		HashArr[hashfunc(idx + probing - 1)].flag = ISITEM;
	}
}
