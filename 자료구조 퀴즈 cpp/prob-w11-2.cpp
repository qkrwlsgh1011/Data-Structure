...
public:
	...
	void tableInsertLinear(int key) {
		...
		while (hashArr[hashfunc(hashfunc(key) + (probing - 1)*hashfunc2(key))].flag == ISITEM) {
			probing += 1;
			if (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == NOITEM) {
				break;
			}
		}
		if (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == NOITEM) {
	...
	void tableSearchLinear(int key) {
		int probing = 1;
		while (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == ISITEM ||
			hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == AVAILABLE) {
		...
		if (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].key == key) {	//못찾음
			cout << 1 << " " << probing << endl;
		}
		else {
			cout << 0 << " " << probing << endl;
	...
	void tableDeleteLinear(int key) {
		...
		while (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == ISITEM ||
			...
			if (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].key == key) {
				hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag = AVAILABLE;
				hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].key = -1;
				break;
			}
			probing += 1;
		}
		...
		if (hashArr[hashfunc(hashfunc(key) + (probing - 1) * hashfunc2(key))].flag == AVAILABLE) {
			cout << 1 << " " << probing << endl;
		}
		else {
			cout << 0 << " " << probing << endl;
