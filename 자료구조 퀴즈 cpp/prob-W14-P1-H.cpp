...
public:
	...
	void insert_edge(int source, int destination, string data) {
		if (isFindVertex(source) == true && isFindVertex(destination) == true) {
			...
			edge* newEdge = new edge(srcVertex, dstVertex, data);		//totaledgelist에 추가될 새로운 edge (E1)

			TotaledgeList->insert(newEdge);

			edge* tempEdge1 = new edge(srcVertex, dstVertex, data);		//src.incidentedges 에 추가될 새로운 edge (E2) 
			edge* tempEdge2 = new edge(srcVertex, dstVertex, data);		//destination.incidentedges에 추가될 새로운 edge(E3)
		...
		if (start->visited == true) {
			...
			while (temp != NULL) {
				...
				if (temp->back == true || temp->discovery == true) {
					...
					if (end->visited == true) {
						temp->myself_InTotal_EdgeList->back = true;	//temp는 incident 엣지이므로 
						temp->myself_InTotal_EdgeList->myself_InFisrt_incidentEdge->back = true;//total을 바꿔주는 과정
